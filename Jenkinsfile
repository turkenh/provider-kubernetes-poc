pipeline {
    ...
    environment {
        ...
        CODECOV_TOKEN = credentials('codecov-provider-kubernetes')
    }
            ...
            steps {
                script {
                    if (env.CHANGE_ID != null) {
                        def json = sh (script: "curl -s https://api.github.com/repos/crossplane-contrib/provider-kubernetes/pulls/${env.CHANGE_ID}", returnStdout: true).trim()
